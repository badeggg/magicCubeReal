...
//参数obj是一个对象，包含着实际的有用的参数
//例：{db: db, username: 'username', email: 'email address@gmail.com', password: '123456'}
	var db = obj.db,
		username = obj.username,
		email = obj.email,
		password = obj.password;
	db.query('INSERT INTO user (username, email, password) VALUES (?, ?, ?)', [username, email, password], function(err, rows){
			if(err){
				throw err;
			}
	});
